...
def judge_channel(channel_num):
    ...
    if channel_num == "channel1":  # 第一通道
        ...
        print('当前是第9通道')
        channel9_mode(channel_num)
    elif channel_num == "channel10":
        print('当前是第10通道')
        channel10_mode(channel_num)
    elif channel_num == "channel11":
        print('当前是第11通道')
        channel11_mode(channel_num)
...
def channel9_mode(channel_num):
    """
    channel9的模式判断，并发送模式配置和随机值
    :param channel_num:
    :return:
    """
    mode_list = ['value', 'current']
...
def channel10_mode(channel_num):
    """
    channel10的模式判断，并发送模式配置和随机值
    :param channel_num:
    :return:
    """
    mode_list = ['value', 'current']
...
def channel11_mode(channel_num):
    """
    channel11的模式判断，并发送模式配置和随机值
    :param channel_num:
    :return:
    """
    mode_list = ['value', 'current']
...
def compare(acc_device, device):
    ...
    :return:
    """
    if abs((acc_device - device)) / device <= 3 / 100:
        print('合格')
    else:
        print('不合格')
...
if __name__ == '__main__':
    ...
    for channel in channel_list:    # 测试所有通道1-15
